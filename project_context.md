...
- Footer repeats the horizontal Autoscape PNG brand mark and includes production contact details, internal quick links, and Privacy/Terms links; quote, auth, payment, confirmation, and dashboard-payment funnel routes use a compact footer variant with those legal links preserved. The public HTML shell also includes no-script Privacy/Terms fallback links for basic crawlers.
...
- Public payment-link tokens are stored hashed and scoped to approved quote payment; when a newer approved email/resend token supersedes an older token, the older emailed URL resolves to the latest payment link rather than creating a separate checkout state.

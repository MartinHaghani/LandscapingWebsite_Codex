...
- Hosted deployment target: DigitalOcean App Platform with isolated staging and production apps.
  ...
  - Live status on 2026-04-21: staging app `autoscape-staging` is active in `tor`, `autoscape-staging-db` is PostgreSQL 16 in `tor1`, and migrations have run. Staging custom domains are blocked until `autoscape.ca` DNS is delegated to DigitalOcean or matching records are added at the current DNS provider. Production has not been created.

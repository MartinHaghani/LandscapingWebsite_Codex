...
- Contact page: `client/src/pages/ContactPage.tsx` (warm-light two-column contact surface with prominent direct phone/email links in rounded panels and the existing idempotent message form)
...
- Public layout shell: `client/src/components/layout/SiteLayout.tsx` (`Navbar`, `Footer`, `ScrollToTop`; navbar/footer use the horizontal PNG brand mark from `client/public/images/brand/autoscape-horizontal-brand.png`, and signed-out desktop auth links use a slim divider rather than punctuation)

...
- Live status on 2026-04-21 keeps this separation intact: staging is active with PostgreSQL 16/PostGIS, custom domains are `CONFIGURING` with `DomainUnexpectedNameserver`, and production is intentionally uncreated until DNS validation and smoke tests pass.

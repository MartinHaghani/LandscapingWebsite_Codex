...
- Services page now presents four real-image service cards for Autonomous Mowing, Smart Edging, Cleanup & Debris, and Performance Reporting.
- `/gallery` displays nine unique before/after lawn transformations from Vaughan and Richmond Hill in a calm responsive image grid.
- Marketing pages now use launch-ready production copy (no placeholder content), warm-light visual tokens, and readability-first spacing/contrast across the home, services, gallery, and contact surfaces, with mobile navigation, a cleaner signed-out header divider, route-aware full/compact footer variants, footer quick links, and a contact page that promotes compact direct phone/email actions beside the message form.
...
- Home page now keeps a tighter top-of-page flow: hero, pricing comparison, mower action video, mower technology section, three-card services overview with Autonomous Mowing, Edging, and Cleanup & Debris, FAQ, and final quote CTA, with flatter post-hero sections, an unboxed larger sample lawn price check with extra intro spacing, a narrower row-label column, a seasonal `20% off` corner badge, an autoplaying muted reliability-focused mower clip from `client/public/videos/home/lawnmower-in-action.mp4` presented as a compact ruled band without native player controls, icon-led mower specs, larger FAQ answers with bolded key phrases, and the older Why Electric, How It Works, Why Autoscape, and Testimonials sections removed. On mobile, the mower asset appears directly beneath the `Meet our lawnmowers` heading before the highlight list.

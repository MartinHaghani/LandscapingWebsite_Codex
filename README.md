...
- Public legal pages are served from `/legal` and `/legal/:slug`, using Markdown sources in `client/src/content/legal/` plus a small raw-Markdown renderer. Footer links expose Privacy and Terms across both full and compact footer variants, the Vite HTML shell includes no-script Privacy/Terms fallback links for basic crawlers, and `/legal` indexes all launch legal documents.

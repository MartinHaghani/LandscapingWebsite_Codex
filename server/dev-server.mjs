...
// server/node_modules/@turf/helpers/dist/esm/index.js
...
// server/node_modules/@turf/meta/dist/esm/index.js
...
// server/node_modules/@turf/area/dist/esm/index.js
...
// server/node_modules/@turf/invariant/dist/esm/index.js
...
// server/node_modules/@turf/boolean-clockwise/dist/esm/index.js
...
var index_default2 = booleanClockwise;

// server/node_modules/@turf/kinks/dist/esm/index.js
...
var index_default3 = kinks;

// server/src/lib/geometry.ts
var MIN_CLOSED_RING_POINTS = 4;
var EARTH_RADIUS_M = 63710088e-1;
var hasSamePoint = (a, b) => a[0] === b[0] && a[1] === b[1];
var toRadians = (value) => value * Math.PI / 180;
var haversineDistanceM = (from, to) => {
...
var normalizeRing = (ring) => {
  if (ring.length < 3) {
    return ring;
  }
  const closed = hasSamePoint(ring[0], ring[ring.length - 1]) ? [...ring] : [...ring, ring[0]];
  if (closed.length >= MIN_CLOSED_RING_POINTS && index_default2(lineString(closed))) {
    return [...closed].reverse();
  }
  return closed;
};
var validateAndMeasurePolygon = (ring) => {
  ...
  const distinctPoints = new Set(normalizedRing.slice(0, -1).map((point2) => point2.join(",")));
  if (distinctPoints.size < 3) {
    throw new Error("Polygon must include at least 3 distinct points.");
  }
  const polygonFeature = polygon([normalizedRing]);
  const kinksResult = index_default3(polygonFeature);
  const selfIntersecting = kinksResult.features.length > 0;
  const areaM2 = index_default(polygonFeature);
...
var coordinateSchema = external_exports.tuple([
  external_exports.number().min(-180).max(180),
  external_exports.number().min(-90).max(90)
]).readonly();
var quotePayloadSchema = external_exports.object({
  address: external_exports.string().trim().min(3).max(300),
  location: external_exports.object({
    lat: external_exports.number().min(-90).max(90),
    lng: external_exports.number().min(-180).max(180)
  }),
  polygon: external_exports.object({
    type: external_exports.literal("Polygon"),
    coordinates: external_exports.array(external_exports.array(coordinateSchema)).min(1)
  }),
  metrics: external_exports.object({
    areaM2: external_exports.number().positive(),
    perimeterM: external_exports.number().positive()
  }),
  plan: external_exports.string().trim().min(3).max(120),
  quoteTotal: external_exports.number().nonnegative()
});
var contactPayloadSchema = external_exports.object({
  name: external_exports.string().trim().min(2).max(100),
  email: external_exports.string().trim().email().max(160),
  message: external_exports.string().trim().min(8).max(5e3)
});
...
var envOrigins = (process.env.CLIENT_ORIGIN ?? "").split(",").map((value) => value.trim()).filter(Boolean);
var allowedOrigins = /* @__PURE__ */ new Set([...defaultOrigins, ...envOrigins]);
var globalWindowMs = 6e4;
var globalLimit = 45;
var writeWindowMs = 6e4;
var writeLimit = 20;
var metricDriftTolerance = 0.03;
var globalRequests = /* @__PURE__ */ new Map();
var writeRequests = /* @__PURE__ */ new Map();
var json = (res, statusCode, payload) => {
...
var withinRateLimit = (requests, key, windowMs, max) => {
  const now = Date.now();
...
var readJson = async (req, limitBytes = 1e6) => {
  const chunks = [];
  let size = 0;

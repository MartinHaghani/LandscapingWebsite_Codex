...
describe('polygonHistory', () => {
  it('pushes the current state to past when applying an edit', () => {
    ...
    const next = applyPolygonEdit(initial, {
      polygons: [{ id: 'p1', kind: 'service', ringPoints: [[-96.8, 32.7]], rawStrokePoints: null }],
      activePolygonId: 'p1'
    });
  ...
  it('undo restores previous state and redo reapplies it', () => {
    ...
    expect(redone.present.polygons).toEqual([
      { id: 'p1', kind: 'service', ringPoints: [[-96.8, 32.7]], rawStrokePoints: null }
    ]);
  });
  ...
  it('clears future history after a branched edit', () => {
    ...
    const branched = applyPolygonEdit(undone, {
      polygons: [{ id: 'p2', kind: 'service', ringPoints: [[-96.82, 32.71]], rawStrokePoints: null }],
      activePolygonId: 'p2'
    });
  ...
  it('treats raw stroke changes as history edits', () => {
    const edited = applyPolygonEdit(createPolygonHistory({ polygons: [], activePolygonId: null }), {
      polygons: [
        {
          id: 'p1',
          kind: 'service',
          ringPoints: [[-96.8, 32.7], [-96.81, 32.71], [-96.82, 32.72]],
          rawStrokePoints: [[-96.8, 32.7], [-96.805, 32.705], [-96.82, 32.72]]
        }
      ],
      activePolygonId: 'p1'
    });

    const changed = applyPolygonEdit(edited, {
      polygons: [
        {
          id: 'p1',
          kind: 'service',
          ringPoints: [[-96.8, 32.7], [-96.81, 32.71], [-96.82, 32.721]],
    ...
    expect(changed.present.polygons[0]?.rawStrokePoints).toBeNull();
  });
});

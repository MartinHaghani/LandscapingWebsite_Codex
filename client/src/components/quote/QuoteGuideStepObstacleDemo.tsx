...
import {
  ...
  QUOTE_GUIDE_DEMO_FRONT_TREE_OBSTACLE_RAW_STROKE_PATH,
  QUOTE_GUIDE_OBSTACLE_DEMO_LOOP_DURATION_MS
} from '../../lib/quoteGuideObstacleDemo';
...
interface QuoteGuideStepObstacleDemoProps {
  ariaLabel: string;
}
...
export const QuoteGuideStepObstacleDemo = ({
  ariaLabel
}: QuoteGuideStepObstacleDemoProps) => {
  ...
  useEffect(() => {
    ...
    const tick = (timestamp: number) => {
      ...
      setElapsedMs((timestamp - startTime) % QUOTE_GUIDE_OBSTACLE_DEMO_LOOP_DURATION_MS);
  ...
  const animationState = useMemo(
    () =>
      prefersReducedMotion
        ? getReducedMotionQuoteGuideObstacleDemoAnimationState()
        : getQuoteGuideObstacleDemoAnimationStateAtElapsedMs(elapsedMs),
  ...
  const rawStrokePath =
    animationState.rawStrokePath ?? QUOTE_GUIDE_DEMO_FRONT_TREE_OBSTACLE_RAW_STROKE_PATH;
  const stageOpacity = prefersReducedMotion
    ? 1
    : getQuoteGuideDemoLoopFadeOpacity(elapsedMs, QUOTE_GUIDE_OBSTACLE_DEMO_LOOP_DURATION_MS);

  return (
    <div
      className="pointer-events-none relative h-full min-h-[25.5rem] w-full"
      data-guide-obstacle-demo="true"
            ...
            >
              ...
              >
                ...
                >
                  <defs>
                    <filter
                      id="quote-guide-obstacle-service-vertex-shadow"
                      x="-120%"
                      y="-120%"
                      width="340%"
                      height="340%"
                    ...
                    <filter
                      id="quote-guide-obstacle-vertex-shadow"
                      x="-120%"
                      y="-120%"
                      width="340%"
                      height="340%"
                  ...
                  {animationState.polygons.map((polygon) => {
                    const polygonStyle = getQuotePolygonStyleToken(
                      polygon.kind,
                      polygon.selected
                    );
                    ...
                    return (
                      <g key={polygon.id}>
                        <path
                          d={buildQuoteGuideObstaclePolygonPath(polygon.ringLocalPoints)}
                        ...
                        {polygon.showVertices
                          ? polygon.ringLocalPoints.map((point, index) => (
                              <QuoteGuideVertexMarker
                                ...
                                cx={point.x}
                                cy={point.y}
                                selected={index === polygon.selectedVertexIndex}
                                kind={polygon.kind}
                                filterId={
                                  polygon.kind === 'obstacle'
                                    ? 'quote-guide-obstacle-vertex-shadow'
                                    : 'quote-guide-obstacle-service-vertex-shadow'
                                }
                              />
                            ))
                          : null}
                ...
                {animationState.showCursor && renderCursorInCamera ? (
                  <QuoteGuideDemoCursor
                    ...
                    showClickPulse={animationState.phase === 'activate-obstacle-draw'}
                    clickTone="danger"
                  />
                ) : null}
              </div>

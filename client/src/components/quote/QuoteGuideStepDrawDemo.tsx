...
import {
  ...
  QUOTE_GUIDE_DEMO_FRONT_ZONE_RAW_STROKE_PATH,
  ...
  type QuoteGuideDemoInstructionState
...
interface QuoteGuideStepDrawDemoProps {
  ariaLabel: string;
  onInstructionStateChange?: (state: QuoteGuideDemoInstructionState) => void;
}

const QUOTE_GUIDE_STEP_DRAW_VIEWPORT_SCALE_CAP = 1;
...
export const QuoteGuideStepDrawDemo = ({
  ariaLabel,
  onInstructionStateChange
}: QuoteGuideStepDrawDemoProps) => {
  ...
  useEffect(() => {
    ...
    const tick = (timestamp: number) => {
      ...
      setElapsedMs((timestamp - startTime) % QUOTE_GUIDE_DEMO_LOOP_DURATION_MS);
  ...
  useEffect(() => {
    ...
    const updateScale = () => {
      ...
      const nextScale = Math.min(
        ...
        QUOTE_GUIDE_STEP_DRAW_VIEWPORT_SCALE_CAP
      );
  ...
  const animationState = useMemo(
    () =>
      prefersReducedMotion
        ? getReducedMotionQuoteGuideDemoAnimationState()
        : getQuoteGuideDemoAnimationStateAtElapsedMs(elapsedMs),
    [elapsedMs, prefersReducedMotion]
  );

  const instructionState = getQuoteGuideDemoInstructionStateForPhase(animationState.phase);
  const rawStrokePath =
    animationState.rawStrokePath ?? QUOTE_GUIDE_DEMO_FRONT_ZONE_RAW_STROKE_PATH;
  ...
  const stageOpacity = prefersReducedMotion
    ? 1
    : getQuoteGuideDemoLoopFadeOpacity(elapsedMs, QUOTE_GUIDE_DEMO_LOOP_DURATION_MS);
  ...
  return (
    <div
      ...
      data-guide-draw-demo="true"
      data-guide-step-scale-cap={QUOTE_GUIDE_STEP_DRAW_VIEWPORT_SCALE_CAP}
          ...
          <div className="relative flex h-full flex-col overflow-hidden bg-white">
            ...
            >
              <QuoteGuideDemoToolbar
                drawButtonLabel={animationState.buttonLabel}
            ...
            >
              ...
              >
                ...
                >
                  <defs>
                    <filter
                      id="quote-guide-demo-vertex-shadow"
                      x="-120%"
                      y="-120%"
                      width="340%"
                      height="340%"
                  ...
                  {animationState.polygons.map((polygon) => {
                    ...
                    return (
                      <g key={polygon.id}>
                        <path
                          d={buildQuoteGuideSvgPath(polygon.ringLocalPoints, true)}
                          ...
                          ? polygon.ringLocalPoints.map((point, index) => {
                              ...
                              return (
                                <QuoteGuideVertexMarker
                                  ...
                                  cy={point.y}
                                  selected={selected}
                                  filterId="quote-guide-demo-vertex-shadow"
                                />
                              );
                            })
                          : null}
                  ...
                  {animationState.rawStrokeProgress > 0 ? (
                    <path
                      d={rawStrokePath}
                      fill="none"
                      stroke="#7DE8A6"
                ...
                {animationState.showCursor && renderCursorInCamera ? (
                  <QuoteGuideDemoCursor
                    ...
                    showClickPulse={
                      animationState.phase === 'activate-draw' ||
                      animationState.phase === 'activate-back-draw'
                    }
                  />
                ) : null}
              </div>
